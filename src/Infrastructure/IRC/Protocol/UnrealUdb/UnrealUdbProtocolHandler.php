 ...
 *                                                   UnrealUdb treats the link as 3.2.x and rejects it
...
class UnrealUdbProtocolHandler extends AbstractProtocolHandler
{
    ...
    /**
     * PROTOCTL capability tokens announced to UnrealUdb.
        ...
        if (count($message->params) >= 3) {
            ...
            if ('INF' === $operation) {
                ...
                if (null !== $this->eventDispatcher) {
                    $this->eventDispatcher->dispatch(new Event\UdbSyncRequestedEvent($block, $message->prefix ?? ''));
                }
            } elseif ('INS' === $operation) {
                $key = $message->params[2];
                $value = $message->params[3] ?? $message->trailing ?? '';
                if (null !== $this->eventDispatcher) {
