...
final readonly class GlineDoctrineRepository implements GlineRepositoryInterface
{
    ...
    public function findByMaskPattern(string $pattern): array
    {
        ...
        $result = $this->em
            ->createQuery(
                'SELECT g FROM App\OperServ\Domain\Entity\Gline g WHERE LOWER(g.mask) LIKE :pattern ORDER BY g.createdAt DESC'
            )
    ...
    public function findExpired(): array
    {
        ...
        $result = $this->em
            ->createQuery(
                'SELECT g FROM App\OperServ\Domain\Entity\Gline g WHERE g.expiresAt IS NOT NULL AND g.expiresAt < :now ORDER BY g.createdAt DESC'
            )
    ...
    public function findActive(): array
    {
        ...
        $result = $this->em
            ->createQuery(
                'SELECT g FROM App\OperServ\Domain\Entity\Gline g WHERE g.expiresAt IS NULL OR g.expiresAt > :now ORDER BY g.createdAt DESC'
            )
            ->setParameter('now', new DateTimeImmutable()->format('Y-m-d H:i:s'))
            ->getResult();
    ...
    {
        return (int) $this->em
            ->createQuery('SELECT COUNT(g.id) FROM App\OperServ\Domain\Entity\Gline g')
    ...
    public function clearCreatorNickId(int $nickId): void
    {
        $this->em
            ->createQuery(
                'UPDATE App\OperServ\Domain\Entity\Gline g SET g.creatorNickId = NULL WHERE g.creatorNickId = :nickId'
            )
