...
final readonly class DeadminCommand implements ChanServCommandInterface
{
    ...
    private function validateDeadminSender(ChanServContext $context, RegisteredChannel $channel, string $channelName, string $targetNick): ?array
    {
        ...
        if (!$context->isLevelFounder) {
            $this->accessHelper->requireLevel($channel, $senderAccount->id, ChannelLevel::KEY_ADMINDEADMIN, $channelName, 'DEADMIN');
        }

        return $this->validateDeadminTarget($context, $channel, $channelName, $targetNick, $senderAccount);
    }

    /** @return array{string, string, RegisteredChannel, SenderView}|null */
    private function validateDeadminTarget(ChanServContext $context, RegisteredChannel $channel, string $channelName, string $targetNick, ChanAccountView $senderAccount): ?array
    {
