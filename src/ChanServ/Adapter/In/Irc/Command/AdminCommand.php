...
final readonly class AdminCommand implements ChanServCommandInterface
{
    ...
    private function validateAdminSender(ChanServContext $context, RegisteredChannel $channel, string $channelName, string $targetNick): ?array
    {
        ...
        if (!$context->isLevelFounder) {
            $this->accessHelper->requireLevel($channel, $senderAccount->id, ChannelLevel::KEY_ADMINDEADMIN, $channelName, 'ADMIN');
        }
    ...
    /** @return array{string, string, RegisteredChannel, SenderView}|null */
    private function validateAdminSecureCheck(ChanServContext $context, RegisteredChannel $channel, string $channelName, string $targetNick, ChanAccountView $targetAccount, SenderView $targetSender): ?array
    {
