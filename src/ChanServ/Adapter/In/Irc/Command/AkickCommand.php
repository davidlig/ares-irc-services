...
final readonly class AkickCommand implements ChanServCommandInterface
{
    ...
    private function doList(ChanServContext $context, RegisteredChannel $channel, string $channelName, ChanAccountView $senderAccount): void
    {
        if (!$context->isLevelFounder) {
            $this->accessHelper->requireLevel($channel, $senderAccount->id, ChannelLevel::KEY_AKICK, $channelName, 'AKICK LIST');
        }
    ...
    private function doAdd(ChanServContext $context, RegisteredChannel $channel, string $channelName, ChanAccountView $senderAccount, SenderView $sender): void
    {
        if (!$context->isLevelFounder) {
            $this->accessHelper->requireLevel($channel, $senderAccount->id, ChannelLevel::KEY_AKICK, $channelName, 'AKICK ADD');
        }
    ...
    private function completeAdd(ChanServContext $context, RegisteredChannel $channel, string $channelName, string $mask, ChanAccountView $senderAccount, SenderView $sender): void
    {
        $expiresAt = null;
        $reason = null;
    ...
    private function doDel(ChanServContext $context, RegisteredChannel $channel, string $channelName, ChanAccountView $senderAccount, SenderView $sender): void
    {
        if (!$context->isLevelFounder) {
            $this->accessHelper->requireLevel($channel, $senderAccount->id, ChannelLevel::KEY_AKICK, $channelName, 'AKICK DEL');
        }
    ...
    private function getProtectedNicks(RegisteredChannel $channel): array
    {
        ...
        // Founder
        $founderNick = $this->accountPort->findNicknameById($channel->getFounderNickId());
        if (null !== $founderNick) {
            $nicks[] = $founderNick;
        }
        ...
        if (null !== $successorId) {
            $successorNick = $this->accountPort->findNicknameById($successorId);
            if (null !== $successorNick) {
                $nicks[] = $successorNick;
            }
        }
        ...
        foreach ($accessList as $access) {
            $nick = $this->accountPort->findNicknameById($access->getNickId());
            if (null !== $nick) {
                $nicks[] = $nick;
            }
        }
    ...
    private function resolveCreatorName(?int $creatorNickId, ChanServContext $context): string
    {
        ...
        $nick = $this->accountPort->findNicknameById($creatorNickId);

        return null !== $nick ? $nick : $context->trans('akick.list.unknown_creator');
    }
