...
final readonly class ChanServNojoinEnforceSubscriber implements EventSubscriberInterface
{
    ...
    private function enforceNojoinForUser(
        ...
        $account = $this->accountPort->findAccountByNick($nick);
        $userLevel = null !== $account && $isIdentified
            ? $this->accessHelper->effectiveAccessLevel($channel, $account->id, true)
            : ChannelAccess::LEVEL_UNREGISTERED;

        if ($userLevel < $nojoinLevel) {
            $language = $account->language ?? $this->defaultLanguage;
    ...
    private function enforceNojoinForChannel(
        ...
        foreach ($view->members as $member) {
            ...
            $account = $this->accountPort->findAccountByNick($user->nick);
            $userLevel = null !== $account && $user->isIdentified
                ? $this->accessHelper->effectiveAccessLevel($channel, $account->id, true)
                : ChannelAccess::LEVEL_UNREGISTERED;
