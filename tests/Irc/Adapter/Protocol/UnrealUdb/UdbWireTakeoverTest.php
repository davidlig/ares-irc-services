...
final class UdbWireTakeoverTest extends TestCase
{
    ...
    #[Test]
    public function onlyAnExactRetransmittedEndOfACompletedBlockIsAcknowledgedAgain(): void
    {
        $digest = $this->stagedTransfer(UdbBlock::Ips, ['1.2.3.4::clones' => '*5']);

        self::assertNull($this->takeover->accept($this->frame(UdbFrameKind::Begin, UdbBlock::Ips, 'tx-I', $digest)));
        self::assertNull($this->takeover->accept($this->frame(UdbFrameKind::End, UdbBlock::Ips, 'wrong', $digest)));
        $completed = $this->takeover->accept($this->frame(UdbFrameKind::End, UdbBlock::Ips, 'tx-I', $digest));
    ...
    #[Test]
    public function inactivityTimeoutDiscardsAnIncompleteStageDeterministically(): void
    {
        $this->begin(UdbBlock::Ips, 'tx1');
        ...
        self::assertSame('', $this->end(UdbBlock::Ips, 'tx1', UdbChecksum::EMPTY));
    }

    #[Test]
    public function activeStageCannotBeReplacedAndLateProgressExpiresIt(): void
    {
        $this->begin(UdbBlock::Ips, 'tx1');
        $this->begin(UdbBlock::Ips, 'tx2');
        self::assertSame(80, $this->takeover->nextDeadline());
    ...
    #[Test]
    public function validPutProgressCannotExtendAStageBeyondItsAbsoluteTimeout(): void
    {
        $this->begin(UdbBlock::Ips, 'tx1');
        foreach ([50, 50, 50, 50, 50] as $advance) {
