...
final class LayerDependencyTest extends TestCase
{
    ...
    #[Test]
    public function domainOnlyImportsItsDomainSharedKernelOrNativePhp(): void
    {
        $violations = [];
    ...
    public function newTopologyOnlyUsesTheModeledContextsAndHexagonalLayers(): void
    {
        ...
        foreach ($entries as $entry) {
            ...
            self::assertContains($entry, $allowedRoots, 'src/' . $entry . ' is not part of the final topology');
        }
    ...
    #[Test]
    public function innerLayersDoNotNameConcreteProtocols(): void
    {
        $actual = [];
        $paths = [];
    ...
    #[Test]
    public function unrealImplementationsNeitherDependOnSiblingsNorShareBehavior(): void
    {
        $actual = [];
        $paths = [
        ...
        self::assertSame([], $actual, 'UnrealStandalone and UnrealUdb must evolve independently without a shared behavioral layer');
    }
    ...
    {
        foreach (['Amp\\', 'Doctrine\\', 'Psr\\', 'Revolt\\', 'Symfony\\'] as $prefix) {
    ...
    private static function protocolNames(): array
    {
        ...
        foreach (['src/Irc/Adapter/Protocol'] as $path) {
